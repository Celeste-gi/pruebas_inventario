...
try:
    logo_path = r"C:\Users\celes\pruebas_inventario\logo.png.jpg"  # Ruta de la imagen del logo
    logo_image = Image.open(logo_path)
    logo_image = logo_image.resize((100, 100), Image.ANTIALIAS)  # Ajustar el tamaño
    logo_photo = ImageTk.PhotoImage(logo_image)
    logo_label = tk.Label(ventana, image=logo_photo, bg="#f2f2f2")
...
boton_seleccionar_archivo = tk.Button(ventana, text="Seleccionar Archivo Excel", command=seleccionar_archivo, bg="#4CAF50", fg="white", font=("Arial", 10, "bold"))
boton_seleccionar_archivo.grid(row=1, column=0, columnspan=2, pady=10)

# Campos de entrada para cada columna
campos = {
    "Bandeja": tk.StringVar(),
...
tk.Label(ventana, text="Seleccionar Bandeja", bg="#f2f2f2", fg="#333", font=("Arial", 10, "bold")).grid(row=2, column=0, padx=10, pady=5, sticky="w")
...
for idx, (columna, variable) in enumerate(campos.items(), start=3):
    tk.Label(ventana, text=columna, bg="#f2f2f2", fg="#333", font=("Arial", 10, "bold")).grid(row=idx, column=0, padx=10, pady=5, sticky="w")
...
boton_añadir = tk.Button(ventana, text="Añadir Producto", command=lambda: actualizar_stock("añadir"), bg="#5c9e6e", fg="white", font=("Arial", 12, "bold"))
...
boton_ver_articulos = tk.Button(ventana, text="Ver Artículos", command=ver_articulos, bg="#4CAF50", fg="white", font=("Arial", 12, "bold"))
